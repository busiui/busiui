...
import { StringUtils } from './stringUtils.service';
import { HttpClient as _HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';
import { cloneDeep as deepCopy } from 'lodash';
import { EcomErrorHandler } from './ecomErrorHandler.service';
import { globalPath } from 'ecbt-lib/constants';
...
export class ToolsService {
  ...
  constructor(
    ...
    protected errorHandler: EcomErrorHandler,
 ...
 */
  ...
  delGlobal(key: any): void {
    ...
    self[globalPath]['params'][key] = null;
  }

  //删除全局变量
  delAllGlobal(): void {
    const self: any = window;
    self[globalPath]['params'] = {};
  }

  //设置全局变量
  setGlobal(key: any, value: any): void {
    const self: any = window;
    self[globalPath]['params'][key] = value;
  }

  //设置全局变量
  getGlobal(key: string): any {
    const self: any = window;
    return self[globalPath]['params'][key];
  }

  //设置全局变量
  getAllGlobal(): any {
    const self: any = window;
    return self[globalPath]['params'];
  }
    ...
    if (arr) {
      arr.forEach((item: any) => {
        ...
        if (key === 'dynFlag') {
          ...
          if (this.stringUtils.IsUndefined(findValue)) {
        ...
        findValue = this.stringUtils.IsNotNUE(findValue) ? findValue : '';
  ...
  replaceParams(params: any, data: any) {
    ...
    for (const i in params) {
      //Array不用替换
      if (this.stringUtils.IsArray(params[i])) {
        res[i] = params[i];
        ...
        if (params[i] === '') {
          ...
          if (this.stringUtils.IsUndefined(res[i])) {
  ...
  valueLogic(rule: string, data: any) {
    ...
    try {
      console.log(tmp);
      const ret = eval(tmp);
      return this.stringUtils.IsNotNUE(ret) ? ret : null;
    }
    catch (e) {
      console.log(e);
  ...
  findkey(obj: any, key: string): any {
    for (const i in obj) {
      ...
      if (i === key) {
        ...
        if (!this.stringUtils.IsUndefined(findV)) {
          return findV;
        }
      }
    }
