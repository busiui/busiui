...
export class StringUtils {
     ...
     */
    static numberToFixed(number: any, n = 2) {
        if (n > 20 || n < 0) {
    ...
    static FormatNumber(args: any, precision = 2) {
     ...
     */
    static DelFormatNumber(args: any) {
        if ((args === null) || (typeof args === "undefined") || (/^\s*$/.test(args))) {
            return args;
        }
        if (typeof args === "string") {
     ...
     * @returns
     */
    static IsNotUndefined(args: any) {
     ...
     * @returns
     */
    static IsDict(args: any) {
        return StringUtils.IsArray(args) && args.length > 0 && !(typeof args[0].dictId === "undefined") && !(typeof args[0].dictName === "undefined");
    }
     ...
     * @returns 处理后的字符串
     */
    static SortString(sortStringValue: any, rex: any) {
        // 进行处理
        if (StringUtils.IsNotNUE(sortStringValue)) {
            var sortStringArray = sortStringValue.split(rex);
            if (StringUtils.IsArray(sortStringArray)) {
     ...
     * @author vito
     */
    static listToFrStringlistToFrString(sqlList: any) {
        var sqlIn = "";
        if (StringUtils.IsArray(sqlList)) {
    ...
    static isContainNumber(val: string): boolean {
